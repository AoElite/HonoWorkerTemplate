...
const run = (
  c: Context<AppEnv>,
  program: Effect.Effect<Response, AppError, Greeting>,
) =>
  Effect.runPromise(
    program.pipe(
      Effect.catchTags({
        SchemaError: (error) =>
          Effect.succeed(
            c.json({ error: 'Bad Request', message: error.message }, 400),
          ),
        MotdNotConfiguredError: () =>
          Effect.succeed(c.json({ error: 'MOTD not configured' }, 500)),
      }),
...
const GreetBody = Schema.fromJsonString(
